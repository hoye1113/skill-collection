...
# get-token.ps1 — Fetch email auth token from credential gateway and write to imap-smtp-email/.env
$ErrorActionPreference = "Stop"

# ── Encoding protection: force UTF-8 without BOM ──
# Windows PowerShell 5.x defaults to system code page (GBK/936), which causes:
#   1. Write-Output Chinese JSON becomes GBK, resolve-account.cjs JSON.parse fails
...
# BUILD_ENV=test uses test environment, otherwise uses production
...
"@
    ...
    # actually writes BOM (EF BB BF), causing Node.js to read .env with BOM prefix on first key
...
function Emit-BestFailureAndExit() {
    ...
    if (-not $selected) {
        $selected = [pscustomobject]@{
            platform = ""
            error_code = 3
            message = "No available email credentials found from credential service. Please log in and connect email capability first, or use setup.sh to configure manually."
        }
    }
...
if ($Token -or $Email) {
    ...
    if ($Token -match '\s') {
        Write-Output (Write-Json $false "Auth token must not contain spaces or newlines" 1 @{ mode = "manual-token" })
        exit 1
    }
    if ($Email -notmatch '@') {
    ...
    Write-Output (Write-Json $true "Personal email credentials written successfully" $null @{ env_path = $EnvFile; mode = "manual-token" })
    exit 0
}

# ── Mode 2: Fetch credentials for specified platform ──
if ($Platform) {
    ...
    if (-not $accessToken -or -not $emailAddress) {
        Write-Output (Write-Json $false "Credential service did not return a valid email address or auth token" 3 @{ platform = $Platform; mode = "platform-specific" })
        exit 1
    }
    ...
    Write-Output (Write-Json $true "Personal email credentials refreshed from credential service" $null @{ env_path = $EnvFile; mode = "platform-specific"; platform = $Platform; email = $emailAddress })
    exit 0
}
...
foreach ($platform in $CheckedPlatforms) {
    ...
    $emailAddress = $response.data.resp.data.extra_data.email_address
    if (-not $accessToken -or -not $emailAddress) {
        Add-Failure $platform 3 "Credential service did not return a valid email address or auth token"
        continue
    }
    ...
    Write-Env $emailAddress $accessToken "credential_service"
    Write-Output (Write-Json $true "Personal email credentials refreshed from credential service" $null @{ env_path = $EnvFile; mode = "credential-service"; platform = $platform; email = $emailAddress })
    exit 0
}
