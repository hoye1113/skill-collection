...
`{workspace}` is your current working directory (provided by the system) — resolve it to that directory; never hardcode an absolute path.
...
run at once (often ~4). The per-facet / per-dimension counts in this skill (e.g.
...
## Kimi CLI Subagent Dispatch

Kimi CLI uses the built-in `Agent` tool rather than ok_computer-style
`create_subagent` plus `task` calls:

- Use `subagent_type="explore"` for wide exploration, dimension deep dives, file
  analysis, and targeted validation.
- Use `subagent_type="plan"` for synthesis or cross-verification if a separate
  reviewing agent is useful.
- Each `Agent.prompt` must include the mission, route, relevant context,
  allowed/disallowed sources, output format, and exact output file path.
- Newly created subagents do not inherit main-agent context. Pass file paths or
  relevant excerpts explicitly; for file-only routes, include the actual excerpts
  needed for the dimension.
- Dispatch in bounded rounds. Use foreground Agent calls when the next phase
  needs the result immediately; use `run_in_background=true` only for independent
  research whose result can be collected later.
...
- Assume internal knowledge may be outdated or incomplete. Always retrieve the current date and time using the Shell tool before any analysis or external search.
- Use Shell to check the time now.
...
2. **Deploy one sub-agent per facet (≈5–8 total), in rounds up to the concurrency limit.** Each sub-agent's prompt MUST include:
...
1. Dispatch one Kimi `Agent(subagent_type="explore")` per dimension — **dispatch in rounds up to the concurrency limit** (fill the limit, await the batch, then continue)
...
1. Dispatch one Kimi `Agent(subagent_type="explore")` per dimension — **dispatch in rounds up to the concurrency limit**
...
3. Each sub-agent's `prompt` MUST include:
   ...
   - **(5) Output file path**: `{workspace}/research/{topic}_dim{NN}.md`
...
**Process**:
1. Read all `{workspace}/research/{topic}_dim{NN}.md` files
...
5. Inline citations `[^id]` must be preserved
6. Conflict Zone analysis must include `[^id]` references to all sources involved
...
1. Verify all required files exist under `{workspace}/research/`:
...
3. Invoke the selected writing skill, providing **explicit file paths** in the handoff:
   ...
   - **Dimension files**: `{workspace}/research/{topic}_dim01.md` through `{topic}_dim{NN}.md`
   - **File analysis** (Route C/D): `{workspace}/research/{topic}_file_analysis.md`
   - **Wide exploration files** (Route A): `{workspace}/research/{topic}_wide{NN}.md`
...
5. **Citation contract** (non-negotiable): every research artifact uses standard Markdown footnotes — `[^id]` in the text plus a `[^id]: Title. Date. URL` definition inline (file-sourced evidence may use a file reference in place of a URL). The writing skill MUST preserve these as-is and let its `md2docx` / `md2pdf` route deduplicate by URL and assign display numbers. Do NOT renumber, do NOT replace them with bare superscript numbers, and do NOT strip definitions into a separate reference list during handoff.
6. **Format ownership**: the writing skill owns final conversion (its `md2docx` / `md2pdf` route). Deep research hands off Markdown only; it does not produce `.docx`/`.pdf` itself.
7. The final document MUST incorporate insights from Phase 6.
8. If the user specified a time range, the orchestrator MUST verify that key data points and dates in the final artifacts fall within that range before handoff.
...
6. **Search budget by route** (targets for total coverage across all rounds, not per simultaneous batch — scale down if the concurrency limit makes the full budget impractical):
...
7. **All outputs must include `[^id]` style citations.**
8. **All files under `{workspace}/research/`.** No exceptions.
