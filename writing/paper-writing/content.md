...
- **Section conventions** → Orchestrator selects the matching Section Writing Conventions block from this file and inlines it into the Agent prompt
- **Citation standards** → Orchestrator copies the specific format rules, density requirements, and source tier definitions into the Agent prompt
...
> below. Either way, every rule here must reach the writer.

Each writer is given the briefing below. The Orchestrator fills every section with **concrete, resolved values**.
...
### Citation Standards
- Format: standard Markdown footnote `[^id]`, immediately after the claim, with the definition `[^id]: Author/Title. Year. URL/DOI` at the end of the section
- **Build footnotes from sources**: take the authors/title, year, and URL/DOI of the work and write them into the definition. Pick a stable `id` (e.g. `firstauthor-year`) and reuse the same `id` whenever you cite that work
- **Reusing research-dimension references**: when incorporating data from research dimension reports (`dim*.md`), carry over the same work (same URL/DOI) under a consistent `[^id]`
...
- Citations inside cells: `98.5% [^src-id]`
...
- **Inline footnote definitions.** End each section file with the `[^id]: Author/Title. Year. URL/DOI` definitions for every `[^id]` it references — one per source. Do not add a separate "References" heading or hand-number anything; assembly merges definitions by URL/DOI and the conversion renders the styled, numbered reference list.
...
For Kimi CLI, dispatch each section via `Agent(subagent_type="coder")`. The task prompt carries ONLY section-specific mission and context. All quality standards are in the prepended writer briefing.
...
## Input Materials
- Research insights: [path to {workspace}/research/{topic}_insight.md — cross-dimension insights; use as the analytical backbone for this section]
...
- Dimension reports: [paths to {workspace}/research/{topic}_dim{NN}.md files relevant to THIS section — detailed source material; Orchestrator selects which dim files apply to each section]
...
| Pattern | Problem | Fix |
|---------|---------|-----|
| "Many researchers have studied X" | Vague, no insight | "Three approaches exist: A[^smith-21], B[^lee-22], C[^chen-23], differing in..." |
