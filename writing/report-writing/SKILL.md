...
3. Dispatch Kimi `Agent(subagent_type="explore")` research sub-agents, one per dimension, each performing ≥5 searches, in bounded rounds
...
1. Dispatch Kimi `Agent(subagent_type="plan")` sub-agents in bounded rounds:
   ...
   - `artifact_analyst` — synthesize research artifacts from `{workspace}/research/`
...
5. **One chapter per Agent call — never merge.**
...
1. A Kimi `Agent(subagent_type="coder")` merger, or the main agent for deterministic assembly, concatenates all `_sec{NN}.md` files (each already carries its own inline footnote definitions) and saves as `{workspace}/{filename}.agent.final.md`
2. **Deduplicate citations by URL**: footnote definitions sharing the same URL are merged; keep each section's `[^id]` references as written (the downstream docx/pdf conversion renumbers by appearance). Identical `[^id]` defs that repeat across sections are collapsed to one.
3. Final validation: cross-references, heading hierarchy, every `[^id]` reference has a matching definition
...
5. **File naming**: `{filename}_sec{NN}.md` for chapters, `{filename}.agent.outline.md` for outline, `{filename}.agent.final.md` for final.
...
9. **Citation integrity.** Writers cite sources with standard Markdown footnotes — `[^id]` in body text plus a `[^id]: Title. Date. URL` definition at the end of their section. Build each footnote directly from the search result's Title / URL / Date. See [citation.md](./citation.md).
...
| [citation.md](./citation.md) | Before Stage 2 and assembly | Standard footnote format, source tiers, URL dedup |
