...
This sub-skill may mention role names such as `drama_writer` or older
...
Before Stage 1, copy all user-uploaded files (reference plays, venue specs, cast lists, set photos) to `{workspace}/research/`. Preserve original filenames. Reference these copies (not originals) in all subagent task prompts — subagents may run in sandboxed environments without access to the original paths.
