...
This sub-skill may mention role names such as `character_writer` or older
...
Before Stage 1, copy all user-uploaded files (venue info, player preferences, theme references) to `{workspace}/research/`. Preserve original filenames. Reference these copies (not originals) in all subagent task prompts — subagents may run in sandboxed environments without access to the original paths.
...
**Save everything** to `{workspace}/{project_name}.truth.md`. This file is the single source of truth for Stage 2. Save the timeline grid to `{workspace}/{project_name}.timeline_grid.md` and the spoiler matrix to `{workspace}/{project_name}.spoiler_matrix.md`.
...
Save to `{workspace}/{project_name}.final.md`. Character scripts to `{workspace}/scripts/`. Host guide to `{workspace}/scripts/{project_name}.host_guide.md`.

### Convert to .docx

Convert the assembled markdown to Word using the `md2docx` pipeline (see the docx skill → `references/md2docx-reference.md`). Character scripts should also be individually converted for print-ready player handouts.
