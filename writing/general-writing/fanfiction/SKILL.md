...
This sub-skill may mention role names such as `fanfic_writer` or older
...
- Use `subagent_type="explore"` for canon/reference intake, research, and read-only analysis.
...
> **CRITICAL — .docx Delivery**: Every completed fanfic must be converted to .docx using the docx skill. Never end a session with raw .md files and no .docx deliverable.
...
Before Stage 1, copy all user-uploaded files (canon excerpts, character references, source material) to `{workspace}/research/`. Preserve original filenames. Reference these copies (not originals) in all subagent task prompts — subagents may run in sandboxed environments without access to the original paths.
...
Create working directory at `{workspace}/`.
...
**Output**: Save to `{workspace}/{project_name}.outline.md`. Orchestrator validates the outline for completeness and consistency, then proceeds automatically.
...
3. For each chapter, dispatch via `task`:
   ...
   - Save each chapter to `{workspace}/chapters/{project_name}.ch{NN}.md`
...
3. Save to `{workspace}/{project_name}.final.md`
4. Convert to .docx using the `md2docx` pipeline (see the docx skill → `references/md2docx-reference.md`). Output: `{workspace}/{project_name}.final.docx`. **This step is mandatory.**
