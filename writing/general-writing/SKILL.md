...
**Paths**: `{workspace}` in any sub-skill is your current working directory (provided by the system) — resolve the placeholder and never hardcode an absolute path.
...
### Main Agent = Planner, Dispatcher, Assembler

Prefer delegating long creative writing, substantive review, and style rewrites to
subagents. The main agent owns planning, routing, prompt construction, file
handoff, progress tracking, assembly, deterministic validation, and final
delivery. Small deterministic fixes, file concatenation, format conversion, and
quality-gate bookkeeping may be done by the main agent when that is simpler and
does not require creative judgment.

Kimi CLI does not support ok_computer-style dynamic `create_subagent` plus
separate `task` calls. Use the built-in `Agent` tool:

- `subagent_type="coder"` for writers, assemblers, file-writing fixers, and
  format-conversion workers.
- `subagent_type="plan"` for outline designers, reviewers, editors, and quality
  gates that should not write files.
- `subagent_type="explore"` for research, reference intake, and read-only
  analysis.
- Put role rules, style constraints, anti-AI rules, input paths, output paths,
  and the task itself into `Agent.prompt`.
- Newly created subagents do not inherit main-agent context. Pass all necessary
  context explicitly, preferably as absolute file paths plus concise inline
  instructions.
- For continuity-heavy work, keep a style bible/state panel on disk and pass it
  every time. If the same subagent instance must continue its own prior context,
  use `resume=<agent_id>`.

> **Delivery note on `system_prompt`**: where this skill says to put rules
> (role, style, anti-AI) into a subagent's `system_prompt`, that means the
> persistent-rules portion of what you send the subagent. In Kimi CLI, fold
> those rules into `Agent.prompt`. The anti-AI rules in particular must reach
> every writer/editor either way.
...
- Dispatch Kimi Agent subagents (writer / editor / planner / researcher)
- Pass context between subagents via explicit prompts and file paths (outlines, state panels, review briefs)
...
**To be clear**: For substantial requests like "write a chapter / draft a full outline / review this / polish that", use Kimi `Agent` subagents unless the task is short enough that delegation overhead would exceed the work. Do not inline long creative output in chat.
...
### Process
9. **Research before writing.** Check `{workspace}/research/` first.
...
- For `.docx` delivery, use the docx skill to convert.
- **Mandatory .docx delivery**: All sub-skills must convert final assembled markdown to `.docx` via the docx skill before the session ends. Raw `.md` without `.docx` is not an acceptable end state.

### Anti-Read-Loop Rule

The orchestrator must dispatch its first Kimi `Agent` subagent within 10 iterations for any substantial writing workflow. If 10+ iterations pass with only file reads and no subagent call, it is in a read-loop. Stop reading, proceed with available information, and dispatch immediately.
...
|------|---------|---------------|------------------------|
| [anti-ai.md](./anti-ai.md) | AI writing pattern avoidance rules | Before creating any **writer** subagent | Main agent reads full text, **inlines into the writer's Agent prompt** — file is < 2000 words, suitable for direct embedding. Also inline into **editor** subagents' prompt as review checklist |
| [review.md](./review.md) | Shared review pipeline: editor subagent definitions and quality gate protocol | Before entering the Review stage | Main agent reads and **follows review.md instructions to dispatch editor subagents** — this file is an operational manual for the main agent, NOT passed to subagents. Each editor's persistent rules and task prompt specs are defined within review.md |
...
|-------------|--------|----------------|-----------|
| Role definition, style rules, anti-AI rules | < 2000 words | Inline text | Agent prompt persistent-rules block |
| Outline, setting docs, state panels | > 2000 words | File path | Agent prompt task block |
| Specific task instructions (chapter #, word count, scene list) | Short | Inline text | Agent prompt task block |
| Predecessor chapter summaries, review briefs | Medium | Inline text | Agent prompt task block |
