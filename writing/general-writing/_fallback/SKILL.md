...
This sub-skill may mention role names such as `writer` or older
...
> **CRITICAL — .docx Delivery**: Every completed work must be converted to .docx using the docx skill. Never end a session with raw .md files and no .docx deliverable.
...
Save to `{workspace}/{filename}.agent.outline.md`. Include: genre name, structural arc, voice profile, audience, occasion.
...
For multi-section works, concatenate all `{filename}_sec{NN}.md` files in order. Save to `{workspace}/{filename}.agent.final.md`.
