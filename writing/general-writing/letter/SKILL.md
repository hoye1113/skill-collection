...
This sub-skill may mention role names such as `letter_writer` or older
...
> **CRITICAL — .docx Delivery**: Every completed letter must be converted to .docx using the docx skill. Never end a session with raw .md files and no .docx deliverable.
...
For letter series or epistolary collections: save full outline to `{workspace}/{filename}.agent.outline.md`.
...
- Output path: `{workspace}/{filename}_sec01.md`
...
Convert the final letter to Word using the `md2docx` pipeline (see the docx skill → `references/md2docx-reference.md`).
