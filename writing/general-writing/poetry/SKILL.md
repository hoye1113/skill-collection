...
This sub-skill may mention role names such as `poetry_writer` or older
...
> **CRITICAL — .docx Delivery**: Every completed poem or collection must be converted to .docx using the docx skill. Never end a session with raw draft files and no .docx deliverable.
...
Save each draft to `{workspace}/{filename}_draft_{A|B|C}.md`.
...
4. Save final poem(s) to `{workspace}/{filename}.agent.final.md`
5. Convert to .docx using the `md2docx` pipeline (see the docx skill → `references/md2docx-reference.md`). Output: `{workspace}/{filename}.agent.final.docx`. **This step is mandatory.** Raw .md without a .docx deliverable is not an acceptable end state.
