...
This sub-skill may mention role names such as `quest_writer` or older
...
> **CRITICAL — .docx Delivery**: Every completed assembly must be converted to .docx using the docx skill. Never end a session with raw .md files and no .docx deliverable.
...
Before Stage 1, copy all user-uploaded files (game design docs, lore bibles, reference materials) to `{workspace}/research/`. Preserve original filenames. Reference these copies (not originals) in all subagent task prompts — subagents may run in sandboxed environments without access to the original paths.
...
**After completion**: Orchestrator reviews the world bible for internal consistency, saves to `{workspace}/{project_name}.outline.md`.
...
- `prompt`: World bible: {world_bible_path}. Detail spec: {detail_spec_path}. Character list: {character_specs}. For each character: produce full profile. Cross-check relationships are bidirectional. Verify no two characters serve the same narrative function. Save to `{workspace}/deliverables/characters/`.
...
```
GW_SKILL_DIR="${KIMI_GENERAL_WRITING_SKILL_DIR:-$(pwd)/.agents/skills/general-writing}"
python "$GW_SKILL_DIR/scripts/check_wordcount.py" <file> --min {target} --lang auto
```
...
Save to `{workspace}/{project_name}.final.md` (single file for small projects) or `{workspace}/deliverables/` subdirectory (for large projects with many deliverables).

**After saving**: Convert the final `.md` to `.docx` using the docx skill. For multi-file deliverables, convert each deliverable document to `.docx`. Never consider assembly complete until `.docx` files are produced.
