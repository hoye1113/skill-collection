...
| File | Role | Path |
|------|------|------|
| Master Outline | Global — foundation, volume structure, plot points, file index, current volume pointer | `{workspace}/{project_name}.outline.md` |
| Volume Outline | Per-volume — chapter status + working window for the active volume | `{workspace}/{project_name}.outline_vol{NN}.md` |
| Outline Raw | Static — full chapter specs archive, written in Stage 1, read-only | `{workspace}/documents/{project_name}.outline_raw.md` |
...
|------|-------------|------|
| Current Volume Outline | Active working outline | `{workspace}/{project_name}.outline_vol02.md` |
| Outline Raw | Full chapter specs archive (read-only) | `{workspace}/documents/{project_name}.outline_raw.md` |
| Characters | Character profiles, living | `{workspace}/documents/{project_name}.characters.md` |
| World | Locations, rules, props | `{workspace}/documents/{project_name}.world.md` |
| Foreshadowing | Plant/payoff registry | `{workspace}/documents/{project_name}.foreshadowing.md` |
| Threads | Plot thread tracker | `{workspace}/documents/{project_name}.threads.md` |
| Summaries | Chapter Done blocks | `{workspace}/documents/{project_name}.chapter_summaries.md` |
...
python "$GW_SKILL_DIR/scripts/check_chapter_quality.py" <chapter_file> --min-words {target} --lang auto
```
