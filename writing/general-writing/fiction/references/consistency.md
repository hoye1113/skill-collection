...
**Step 4**: Write volume summary to `{workspace}/documents/{project_name}.vol{N}_summary.md`. The fiction_writer reads this file (referenced in outline.md or task prompt path) as additional context when starting the next volume. Do NOT inline the volume summary into the task prompt — pass the file path.
