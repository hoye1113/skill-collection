...
```bash
# Single source
uv run python scripts/fetch_news.py --source <source_key> --no-save

# Multiple sources (comma-separated)
uv run python scripts/fetch_news.py --source hackernews,github,wallstreetcn --no-save

# All sources (broad scan)
uv run python scripts/fetch_news.py --source all --limit 15 --deep --no-save

# With keyword filter (auto-expand: "AI" → "AI,LLM,GPT,Claude,Agent,RAG")
uv run python scripts/fetch_news.py --source hackernews --keyword "AI,LLM,GPT" --deep --no-save
```
...
```bash
uv run python scripts/daily_briefing.py --profile <profile>
```
...
- Python 3.8+, `uv pip install -r requirements.txt`
