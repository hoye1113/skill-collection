...
| # | 名称 | 命令 |
|---|---|---|
| 30 | 🌅 综合早报 (General) | `uv run python scripts/daily_briefing.py --profile general --no-save` |
| 31 | 💰 财经早报 (Finance) | `uv run python scripts/daily_briefing.py --profile finance --no-save` |
| 32 | 🤖 科技早报 (Tech) | `uv run python scripts/daily_briefing.py --profile tech --no-save` |
| 33 | 🍉 吃瓜早报 (Social) | `uv run python scripts/daily_briefing.py --profile social --no-save` |
| 34 | 🧠 AI 深度日报 (AI Daily) | `uv run python scripts/daily_briefing.py --profile ai_daily --no-save` |
| 35 | 📚 深度阅读清单 | `uv run python scripts/daily_briefing.py --profile reading_list --no-save` |
