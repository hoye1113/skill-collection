...
> 使用方法：将下方 Prompt 复制到任何支持长上下文的模型中，按提示输入议题即可开始多角色深度对话。
>
> **作者**：李继刚
> **来源**：[GitHub Gist](https://gist.github.com/lijigang/a8f9cf12985d474cef15cda63f4e1892)
...
在对话过程中，可使用以下指令控制讨论走向：

| 指令 | 作用 |
|------|------|
| `可` | 接受主持人提出的下一个深层问题，继续推进 |
| `止` | 终止讨论，生成知识网络总结 |
| `深入此节` | 暂停推进，围绕当前争议点进一步深挖 |
| `引入新人物` | 邀请一位新的代表人物加入讨论 |

## 方法论说明

该 Prompt 采用 Lisp 风格的伪代码定义了一个完整的圆桌研讨会框架，包含主持人（Moderator）和代表人物（Representative）两类角色。核心机制是「辩证流程」：每轮讨论后由主持人提炼核心矛盾，生成 ASCII 思考框架，并基于矛盾提出更深层的问题，逐步逼近议题本质。
