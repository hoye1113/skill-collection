...
```typescript
{
  ...
  image: "",       // Path from public/, e.g. "/images/work-1.jpg".
                   // MUST be a topic-relevant image. Generate it only if the
                   // current client exposes an image-generation tool, or
...
- **`galleryConfig.works[].image`** — one image per work card. These appear both on the grid (480×320 crop) and on the work detail page (1400×933 crop), so favor 3:2 landscape at ~1600×1067 or larger, JPEG. Every image MUST be topic-relevant to the work it illustrates. There are two acceptable sources:
  1. **Generated** — if the current client exposes an image-generation tool, call it with a prompt specific to that work (artist / subject / medium), save into `public/images/`, then set `image: "/images/work-XX.jpg"`.
...
1. **Hero image** — write a prompt for a single painterly / textured image. If the current client exposes an image-generation tool, use it and save as `.jpg` into `public/images/` (e.g. `public/images/hero-source.jpg`); otherwise use a topic-relevant sourced asset. Then set `heroConfig.fluidImagePath = "/images/hero-source.jpg"`.
2. **Instant video** — write a prompt for a ~10-second dark abstract loop. If the current client exposes a video-generation tool, use it and save into `public/videos/ambient.mp4`; otherwise leave `instantConfig.videoPath` empty or use a user-provided/sourced video. Then set `instantConfig.videoPath = "/videos/ambient.mp4"` only when the file exists.
3. **Gallery cards** — one image per work. For each `WorkItem`, either use an available image-generation tool with a prompt tailored to that work's subject (artist, scene, medium), or download a topic-relevant asset. Save to `public/images/` (e.g. `work-1.jpg`, `work-2.jpg`, ...) and set the corresponding `image` field to `"/images/work-N.jpg"`. NEVER use placeholder/random image services — the page must look coherent with the user's topic.
