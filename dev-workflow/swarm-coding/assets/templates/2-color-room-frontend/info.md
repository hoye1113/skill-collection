...
If the required video assets do not already exist, write video-generation prompts based on the user's request and this template's visual style. If the current client exposes a video-generation tool, use it and save generated files into `public/videos/`; otherwise use user-provided/sourced videos or leave optional video fields empty. Reference final file paths in `src/config.ts`.
