...
When this template requires image or video assets, write descriptive prompts. If the current client exposes image/video generation tools, use them; otherwise use user-provided or sourced assets. Save final assets into `public/images/` or `public/videos/`, and reference those final paths in `src/config.ts`. For the shader-heavy aesthetic, prefer dark/moody architectural / installation / light-study photography for gallery images.
