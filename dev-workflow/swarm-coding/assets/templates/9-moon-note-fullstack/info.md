...
**None.** Every background is procedural (WebGL shaders + Canvas 2D). The moon widget is canvas-rendered. Notes are plain Markdown — no image pipeline needed. If a user explicitly asks for photos inside notes and the current client exposes an image-generation tool, use it and drop the file into `public/images/`; otherwise use user-provided or sourced images. By default this template ships without any binary assets.
