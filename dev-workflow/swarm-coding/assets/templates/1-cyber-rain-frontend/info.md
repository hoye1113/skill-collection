...
If the required image assets do not already exist, write image-generation prompts based on the user's request and this template's visual style. If the current client exposes an image-generation tool, use it and save generated files into `public/images/`; otherwise use user-provided or sourced assets. Reference final file paths in `src/config.ts`.
...
If the required video asset does not already exist, write a video-generation prompt based on the user's request and this template's visual style. If the current client exposes a video-generation tool, use it and save the generated file into `public/videos/`; otherwise use a user-provided/sourced video or leave the video field empty when the template supports it. Reference final file paths in `src/config.ts`.
