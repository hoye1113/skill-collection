...
Kimi CLI uses the built-in `Agent` tool rather than ok_computer-style dynamic
subagent creation:

- Use `subagent_type="explore"` for search, URL discovery, pattern discovery,
  and validation that should not write downloaded files.
- Use `subagent_type="coder"` for actual downloading, parsing, file validation,
  and manifest writing under `{workspace}/output/`.
- Put the subtask goal, constraints, candidate URLs, output directory, expected
  schema, and verification requirements into `Agent.prompt`.
- Newly created subagents do not inherit main-agent context. Pass all URLs,
  source notes, cookies/headers constraints if any, and output paths explicitly.
- Dispatch independent subtasks in bounded rounds, normally 3-5 agents at a
  time. Use `run_in_background=true` only when the orchestrator can continue
  without the result and will later collect the task output.
...
- **Parallel**: when searching across different years, sites, domains, file types, or query formulations — launch separate search sub-agents for each
...
Launch one Kimi `Agent` per atomic unit via the Agent tool. Each sub-agent's prompt MUST include:
...
- When subtasks are **independent** (different years, regions, categories, or URLs), delegate them in bounded parallel rounds — one Agent tool invocation per sub-agent, up to the runtime concurrency limit
- Treat each independent unit (year, region, category, or single file URL) as one atomic task handled by one sub-agent
- Example: For reports from 10 years → plan 10 atomic sub-agent assignments, then launch them in batches rather than assuming 10 simultaneous executions
...
2. **Parallelize independent work in bounded rounds.** Different files, years, regions, or categories are delegated to one sub-agent per atomic unit, but simultaneous execution is capped by the runtime.
...
- All downloaded files saved to: `{workspace}/output/`
