---
name: web-visual-artifacts
description: |
  Create standalone visual Web artifacts whose primary goal is presentation, exploration, or demonstration rather than integration into an existing product codebase.
  Use this skill for:
  - HTML landing page concepts or standalone marketing microsites
  - Interactive prototypes and click-through mockups
  - HTML slide decks / presentations
  - Animated demos, motion studies, and timeline-driven showcases
  - Data-visualization showcases or explorable demos
  - Design-system exploration canvases and visual concept studies
  Typical inputs: PRDs, screenshots, Figma frames, moodboards, copy docs, or rough ideas.
  Typical outputs: self-contained HTML/CSS/JS/React artifacts for review, presentation, or user testing.
  Not applicable:
  - Auditing or reviewing existing UI code
  - Production hardening for accessibility, semantics, or performance
  - Routine component/page work inside an existing app or design system
  - Incremental polish to an existing product interface
---

# Web Visual Artifacts
...
Before continuing, verify the request is for a standalone artifact rather than product UI work:

- If the task is to audit or review existing UI code, stop and use `frontend-interface-design`.
- If the task must be merged into an existing app, design system, or production repo, stop and use `frontend-interface-design`.
- If the goal is a concept, prototype, deck, demo, showcase, or self-contained HTML artifact, continue here.

---

## Scope

✅ **Applicable**: Standalone visual artifacts (landing page concepts / prototypes / slide decks / showcases / visualizations / animations / concept studies)

❌ **Not applicable**: Existing product UI audits, in-repo component/page work, production hardening, back-end APIs, CLI tools, data-processing scripts, and other non-visual tasks
...
| "Make a click-through onboarding prototype for my food-delivery app" | ✅ Ask heavily: users, flows, brand, variants |
| "Turn this moodboard into a standalone marketing microsite concept" | ❌ Enough info — start building |
...
- **Output type**: Standalone landing page / prototype / slide deck / animation / dataviz showcase? Fidelity level?
...
#### When Extending an Existing Visual System

When extending an existing site, prototype, or visual system, **understand the visual vocabulary first, then act** — think out loud about your observations so the user can validate your reading:
