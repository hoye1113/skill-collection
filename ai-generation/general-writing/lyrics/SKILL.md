...
This sub-skill may mention role names such as `lyrics_writer` or older
...
> **CRITICAL — .docx / .txt Delivery**: Every completed song must be converted to .docx using the docx skill. For Suno/AI music platform targets, also produce a `.txt` file with meta-tags. Never end a session with only raw section files and no assembled deliverable.
...
- Save each to `{workspace}/{filename}_chorus_{A|B|C}.md`
...
- Save to `{workspace}/{filename}_sec{NN}.md`
...
3. Save to `{workspace}/{filename}.agent.final.md`
4. Convert to .docx using the `md2docx` pipeline (see the docx skill → `references/md2docx-reference.md`). Output: `{workspace}/{filename}.agent.final.docx`. For Suno/AI platform targets, the `.suno.txt` file is also a primary deliverable.
