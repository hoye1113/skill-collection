...
This sub-skill may mention role names such as `narrative_writer` or older
...
Before Stage 1, copy all user-uploaded files (system rulebooks, setting references, existing campaign notes) to `{workspace}/research/`. Preserve original filenames. Reference these copies (not originals) in all subagent task prompts — subagents may run in sandboxed environments without access to the original paths.
...
Save to `{workspace}/{project_name}.outline.md`. Proceed to Stage 2 automatically.
