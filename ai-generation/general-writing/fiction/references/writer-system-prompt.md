...
Orchestrator reads this file and composes the persistent-rules block for the `fiction_writer` Agent prompt by concatenating all sections below into a single string. Sections marked **[INLINE FROM ...]** require reading the referenced file and pasting its content into the prompt.
