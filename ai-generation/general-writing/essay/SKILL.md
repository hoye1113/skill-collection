...
This sub-skill may mention role names such as `essay_writer` or older
...
Orchestrator dispatches all creation and review work to subagents. Before creation, copy user-uploaded files to `{workspace}/research/`.
...
> **CRITICAL — .docx Delivery**: Every completed essay must be converted to .docx using the docx skill. Never end a session with raw .md files and no .docx deliverable.
...
**For short essays (< 3k words):** Save a single outline to `{workspace}/{filename}.agent.outline.md`. Include: central claim, the turn, voice profile, structural arc, platform conventions (if any).

**For long essays (>= 3k words):** Two-phase outline process:
1. **Structural outline** — Central claim, the turn, section purposes, and overall arc. Save to `{workspace}/{filename}.agent.outline.md`.
...
- Output path: `{workspace}/{filename}_sec{NN}.md`
...
For multi-section essays, concatenate all section files in order. Save to `{workspace}/{filename}.agent.final.md`.

### Convert to .docx

Convert the assembled markdown to Word using the `md2docx` pipeline (see the docx skill → `references/md2docx-reference.md`).

Output: `{workspace}/{filename}.agent.final.docx`
